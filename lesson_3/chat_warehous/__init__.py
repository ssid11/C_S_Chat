...
class Server:
    def __init__(self, address=Constants.DEFAULT_IP_ADDRESS, port=Constants.DEFAULT_PORT,
                 proto = Constants.DEFAULT_PROTO, soc_type=Constants.DEFAULT_TYPE):
...
class Client:
    ...
    def Exchange(self, to_server):
        send_mesages(self.socket, to_server)
        print(f'На сервер отправлено сообщение :{to_server}')
        try:
            message = get_messages(self.socket)
    ...
    def Greetings(self):
        self.Exchange( create_greetings())
